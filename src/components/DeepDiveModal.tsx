...
function parseBullets(content: string): { bullets: string[]; context: string } {
  ...
  // If no bullets found, split prose into sentences as bullets
  if (bullets.length === 0) {
    const sentences = content.split(/(?<=[.!?])\s+/).filter(s => s.trim().length > 10);
    const taken = sentences.slice(0, 4);
    const rest = sentences.slice(4);
    return { bullets: taken, context: rest.join(' ') };
  }

  return { bullets: bullets.slice(0, 5), context: contextLines.slice(0, 2).join(' ') };
}
...
export function DeepDiveModal({ idea, report, onClose, onBuildRoom }: DeepDiveModalProps) {
  ...
  const sections = [
    { title: 'Competitive Landscape', content: report.competitiveLandscape, icon: Globe, color: 'text-blue-500' },
    { title: 'Commissioner Fit', content: report.commissionerFit, icon: Sparkles, color: 'text-amber-500' },
    { title: 'Target Audience', content: report.audience, icon: Users, color: 'text-emerald-500' },
    { title: 'Talent & Access', content: report.talentAccess, icon: UserCheck, color: 'text-purple-500' },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto animate-fade-in shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className={`${verdict.bg} px-8 py-8`}>
          ...
          <p className="text-primary-foreground/90 text-base md:text-lg mt-3 leading-relaxed max-w-2xl font-medium">{report.verdictRationale}</p>
        </div>
        ...
        <div className="p-6 space-y-4">
          {sections.map((section, idx) => {
            ...
            const { bullets, context } = parseBullets(section.content);
            return (
