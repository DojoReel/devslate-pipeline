  ...
  if (!report) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
        ...
        <p className="text-sm font-semibold text-foreground">No Deep Dive report yet</p>
        <p className="text-xs mt-1">Run Deep Dive from the Pipeline view first.</p>
      </div>
    );
  }

  const sections = [
    { title: 'Verdict', content: `**${report.verdict}** — ${report.verdictReason}` },
    { title: 'Story Verified', content: report.storyVerified ? `✅ Verified — ${report.verifiedDetail}` : `❌ Not verified — ${report.verifiedDetail}` },
    { title: 'Full Story', content: report.fullStory },
    { title: 'People & Access', content: report.people },
    { title: 'Archive & Footage', content: report.archive },
    { title: 'Rights', content: report.rightsDetail },
    { title: 'Commission Check', content: report.commissionCheck },
    { title: 'Broadcaster Fit', content: report.broadcasterFit },
    { title: 'Format Recommendation', content: report.formatRecommendation },
    { title: 'Why Now', content: report.whyNow },
    { title: 'Red Flags', content: report.redFlags },
    { title: 'Sources', content: report.sources },
  ].filter(s => s.content && s.content.trim());

  return (
    <div className="space-y-4">
      {sections.map(section => (
        <div key={section.title} className="border border-border rounded-xl p-4 md:p-5 bg-background">
          <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">{section.title}</h4>
...
function BuildRoomIdeaCard({ idea }: { idea: PipelineIdea }) {
  ...
  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-md">
      ...
      {/* Tabs + Content */}
      <div className={`${docsExpanded ? 'block' : 'hidden md:block'}`}>
        {/* Tab bar */}
        <div className="flex border-b border-border mx-4 md:mx-6">
          <button
        ...
        <div className="p-4 md:p-6 pt-4">
          {activeTab === 'deep-dive' ? (
            ...
            <>
              ...
              >
                {docsVisible ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                {docsVisible ? 'Hide Documents' : 'Show Documents'}
              </button>
                            ...
                            {(doc.status === 'pending' || doc.status === 'error') && !isGenerating && (
                              idea.report ? (
                                <button
                                  ...
                                  className="px-3 md:px-4 py-2 min-h-[44px] rounded-full bg-primary text-primary-foreground text-xs font-bold hover:scale-105 transition-transform disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                                  Generate
                                </button>
                              ) : (
                                <span className="text-[10px] md:text-xs text-primary font-medium">Run Deep Dive first</span>
                              )
                            )}
                          </div>
                        </div>
